...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        console.error('Error initializing guidance alerts:', error);
    }
});
...
function startRealTimeAlertPolling() {
    ...
    console.log('🔄 Starting guidance alert polling (every 3 seconds)');
    ...
    pollingInterval = setInterval(() => {
        console.log('⏰ Polling for guidance alerts...');
        fetchAlertCounts();
    }, POLL_INTERVAL);
...
async function fetchAlertCounts() {
    try {
        const response = await fetch('/guidance/alerts/counts', {
            method: 'GET',
            headers: {
        ...
        if (data.success) {
            console.log('📊 Guidance alert counts received:', data.counts);
            updateAlertBadges(data.counts);
        }
    } catch (error) {
        console.error('Error fetching guidance alert counts:', error);
    }
}

function updateAlertBadges(counts) {
    updateBadge('counseling-sessions-link', counts.recommended_counseling || 0);
}
