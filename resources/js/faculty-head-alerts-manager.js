...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        console.error('Error initializing faculty head alerts:', error);
    }
});
...
function startRealTimeAlertPolling() {
    ...
    console.log('🔄 Starting faculty head alert polling (every 3 seconds)');
    ...
    pollingInterval = setInterval(() => {
        console.log('⏰ Polling for faculty head alerts...');
        fetchAlertCounts();
    }, POLL_INTERVAL);
...
async function fetchAlertCounts() {
    try {
        const response = await fetch('/faculty-head/alerts/counts', {
            method: 'GET',
            headers: {
        ...
        if (data.success) {
            console.log('📊 Faculty head alert counts received:', data.counts);
            updateAlertBadges(data.counts);
        }
    } catch (error) {
        console.error('Error fetching faculty head alert counts:', error);
    }
}

function updateAlertBadges(counts) {
    updateBadge('grades-link', counts.pending_grades || 0);
}
