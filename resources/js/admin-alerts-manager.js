...
async function fetchAlertCounts() {
    try {
        const response = await fetch('/admin/alerts/counts', {
            method: 'GET',
            headers: {
...
function updateAlertBadges(counts) {
    updateBadge('contact-messages-link', counts.unread_messages || 0);
    updateBadge('forwarded-cases-link', counts.pending_cases || 0);
}
