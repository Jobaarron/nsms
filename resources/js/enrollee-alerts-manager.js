...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        console.error('Error initializing enrollee alerts:', error);
    }
});
...
function startRealTimeAlertPolling() {
    ...
    console.log('🔄 Starting enrollee alert polling (every 3 seconds)');
    ...
    pollingInterval = setInterval(() => {
        console.log('⏰ Polling for enrollee alerts...');
        fetchAlertCounts();
    }, POLL_INTERVAL);
...
async function fetchAlertCounts() {
    try {
        const response = await fetch('/enrollee/alerts/counts', {
            method: 'GET',
            headers: {
        ...
        if (data.success) {
            console.log('📊 Enrollee alert counts received:', data.counts);
            updateAlertBadges(data.counts);
        }
    } catch (error) {
        console.error('Error fetching enrollee alert counts:', error);
    }
}

function updateAlertBadges(counts) {
    updateBadge('notices-link', counts.unread_notices || 0);
}
